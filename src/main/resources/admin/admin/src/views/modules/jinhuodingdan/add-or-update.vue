<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="图书单价" prop="tushudanjia">
          <el-input v-model="ruleForm.tushudanjia" 
              placeholder="图书单价" clearable  :readonly="ro.tushudanjia"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="图书单价" prop="tushudanjia">
              <el-input v-model="ruleForm.tushudanjia" 
                placeholder="图书单价" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jinhuodingdan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jinhuodingdanCrossAddOrUpdateFlag = false;
                  this.parent.search();
